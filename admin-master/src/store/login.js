...
export const actions = {
  async login({ commit, dispatch/* , state */ }, { username, password }) {
    ...
    try {
      ...
      let res = await this.$axios.$post('/master/api/login', {username, password})
      console.log(res);
  ...
  async fetch({ commit/* , state */ }, { username }) {
    ...
    const res = await this.$axios.$get('/master/api/userinfo', { username })
    const { userinfo } = res;
  ...
  async logout({ commit }) {
    // const res = await auth.fetch({ username });
    const res = await this.$axios.$post('/master/api/logout')
    if (res.errcode && res.errcode === 0) {
